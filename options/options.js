...
options.initMenu = function () {
	...
	sideBarLinks.forEach(function (elem) {
		elem.addEventListener("click", function (e) {
			...
			const activatedTab = $(
				"div.tab#tab-" + elem.getAttribute("href").substring(1),
			);
...
options.initGeneralSettings = function () {
	const changeCheckboxValue = async function (e) {
		...
		if (name === "autoFillAndSend") {
			...
			const message = updated.autoReconnect
				? "enable_automatic_reconnect"
				: "disable_automatic_reconnect";
	...
	if (options.settings["colorTheme"] === undefined) {
		...
		$("#tab-general-settings select#colorTheme").value =
			options.settings["colorTheme"];
	}

	const generalSettingsCheckboxes = document.querySelectorAll(
	...
	$("#tab-general-settings input[type=radio]#checkUpdateThreeDays").value =
		CHECK_UPDATE_THREE_DAYS;
	$("#tab-general-settings input[type=radio]#checkUpdateOneWeek").value =
		CHECK_UPDATE_ONE_WEEK;
	$("#tab-general-settings input[type=radio]#checkUpdateOneMonth").value =
		CHECK_UPDATE_ONE_MONTH;
	$("#tab-general-settings input[type=radio]#checkUpdateNever").value =
		CHECK_UPDATE_NEVER;

	$("#tab-general-settings input[type=range]").value =
		options.settings["redirectAllowance"];
	$("#redirectAllowanceLabel").textContent = tr(
		"optionsRedirectAllowance",
		options.settings["redirectAllowance"] === 11
			? "Infinite"
			: String(options.settings["redirectAllowance"]),
	);

	$("#tab-general-settings select#credentialSorting").value =
		options.settings["credentialSorting"];
	$("#tab-general-settings select#afterFillSorting").value =
		options.settings["afterFillSorting"];
	$("#tab-general-settings select#afterFillSortingTotp").value =
		options.settings["afterFillSortingTotp"];
	$("#tab-general-settings input#defaultGroup").value =
		options.settings["defaultGroup"];
	$("#tab-general-settings input#clearCredentialTimeout").value =
		options.settings["clearCredentialsTimeout"];

	const generalSettingsRadioInputs = document.querySelectorAll(
		"#tab-general-settings input[type=radio]",
	);
	...
	$("#tab-general-settings select#colorTheme").addEventListener(
		"change",
		async function (e) {
	...
	$("#tab-general-settings select#credentialSorting").addEventListener(
		"change",
		async function (e) {
	...
	$("#tab-general-settings select#afterFillSorting").addEventListener(
		"change",
		async function (e) {
	...
	$("#tab-general-settings select#afterFillSortingTotp").addEventListener(
		"change",
		async function (e) {
	...
	$("#tab-general-settings input#clearCredentialTimeout").addEventListener(
		"change",
		async function (e) {
	...
	$("#tab-general-settings input[type=range]").addEventListener(
		"input",
		function (e) {
			const currentValue =
				e.target.valueAsNumber === 11 ? "Infinite" : e.target.value;
			$("#redirectAllowanceLabel").textContent = tr(
				"optionsRedirectAllowance",
				currentValue,
			);
	...
	$("#tab-general-settings button.checkUpdateKeePassXC").addEventListener(
		"click",
		function (e) {
			e.preventDefault();
			e.disabled = true;
	...
	$("#configureCommands").addEventListener("click", function () {
		...
		browser.tabs.create({
			url: isFirefox()
				? browser.runtime.getURL("options/shortcuts.html")
				: `${scheme}://extensions/shortcuts`,
		});
	});
	...
	const dialogImportSettingsModal = new bootstrap.Modal(
		...
		link.onchange = function (e) {
			...
			reader.onloadend = function (ev) {
				try {
					...
					if (
						contents["checkUpdateKeePassXC"] === undefined ||
						contents["autoCompleteUsernames"] === undefined ||
						contents["autoFillAndSend"] === undefined
					) {
						console.log(
							"Error: Not a KeePassXC-Browser settings file.",
						);
						return;
					}
					...
					$("#dialogImportSettings").addEventListener(
						"shown.bs.modal",
						function (modalEvent) {
							modalEvent.currentTarget
								.querySelector(".modal-footer .btn-success")
								.focus();
						},
					);
				} catch (err) {
	...
	$("#exportSettingsButton").addEventListener("click", function () {
		...
		const file = new Blob([JSON.stringify(options.settings)], {
	...
	$("#dialogImportSettings .modal-footer button.yes").addEventListener(
		"click",
		function (e) {
...
options.showKeePassXCVersions = async function (response) {
	...
	$("#tab-general-settings .kphVersion span.yourVersion").textContent =
		response.current;
	$("#tab-general-settings .kphVersion span.latestVersion").textContent =
		response.latest;
...
options.initConnectedDatabases = function () {
	const dialogDeleteConnectedDatabaseModal = new bootstrap.Modal(
		$("#dialogDeleteConnectedDatabase"),
		{ keyboard: true, show: false, backdrop: true },
	);
	...
	$(
		"#dialogDeleteConnectedDatabase .modal-footer button.yes",
	...
	const removeButtonClicked = function (e) {
		...
		$("#dialogDeleteConnectedDatabase").setAttribute(
			"hash",
			closestTr.getAttribute("hash"),
		);
		$("#dialogDeleteConnectedDatabase").setAttribute(
			"tr-id",
			closestTr.getAttribute("id"),
		);
		$("#dialogDeleteConnectedDatabase .modal-body strong").textContent =
			closestTr.children[0].textContent;

		dialogDeleteConnectedDatabaseModal.show();
		$("#dialogDeleteConnectedDatabase").addEventListener(
			"shown.bs.modal",
			function (modalEvent) {
				modalEvent.currentTarget
					.querySelector(".modal-footer .btn-success")
	...
	const rowClone = $("#tab-connected-databases table tr.clone").cloneNode(
		true,
	);
	...
	const addHashToTable = function (hash) {
		...
		row.children[1].textContent = options.getPartiallyHiddenKey(
			options.keyRing[hash].key,
		);
	...
	$("#connect-button").addEventListener("click", async function () {
		...
		if (result === AssociatedAction.NEW_ASSOCIATION) {
			...
			options.keyRing = await browser.runtime.sendMessage({
				action: "load_keyring",
			});
			...
			for (const hash in hashList) {
				const newHash = Object.keys(options.keyRing).find(
					(h) => h !== hash,
				);
...
options.initCustomLoginFields = function () {
	const dialogDeleteCustomLoginFieldsModal = new bootstrap.Modal(
		$("#dialogDeleteCustomLoginFields"),
		{ keyboard: true, show: false, backdrop: true },
	);
	...
	const removeButtonClicked = function (e) {
		...
		const closestTr = this.closest("tr");
		$("#dialogDeleteCustomLoginFields").setAttribute(
			"url",
			closestTr.getAttribute("url"),
		);
		$("#dialogDeleteCustomLoginFields").setAttribute(
			"tr-id",
			closestTr.getAttribute("id"),
		);
		$("#dialogDeleteCustomLoginFields .modal-body strong").textContent =
			closestTr.children[0].textContent;

		dialogDeleteCustomLoginFieldsModal.show();
		$("#dialogDeleteCustomLoginFields").addEventListener(
			"shown.bs.modal",
			function (modalEvent) {
				modalEvent.currentTarget
					.querySelector(".modal-footer .btn-success")
	...
	$(
		"#dialogDeleteCustomLoginFields .modal-footer button.yes",
...
options.initSitePreferences = function () {
	...
	const removeButtonClicked = function (e) {
		...
		$("#dialogDeleteSite .modal-body strong").textContent =
			closestTr.children[0].textContent;

		dialogDeleteSiteModal.show();
		$("#dialogDeleteSite").addEventListener(
			"shown.bs.modal",
			function (modalEvent) {
				modalEvent.currentTarget
					.querySelector(".modal-footer .btn-success")
	...
	const addNewRow = function (
		...
		row.children[1].children[0].addEventListener(
			"change",
			selectionChanged,
		);
		row.children[2].children["usernameOnly"].checked = usernameOnly;
		row.children[2].children["usernameOnly"].addEventListener(
			"change",
			checkboxClicked,
		);
		row.children[3].children["improvedFieldDetection"].checked =
			improvedFieldDetection;
		row.children[3].children["improvedFieldDetection"].addEventListener(
			"change",
			checkboxClicked,
		);
		row.children[4].addEventListener("click", removeButtonClicked);
	...
	$("#dialogDeleteSite .modal-footer button.yes").addEventListener(
		"click",
		function (e) {
			...
			for (
				let i = 0;
				i < options.settings["sitePreferences"].length;
				++i
			) {
	...
	$("#sitePreferencesManualAdd").addEventListener("click", function (e) {
		...
		const rowClone = $("#tab-site-preferences table tr.clone").cloneNode(
			true,
		);
		rowClone.classList.remove("clone", "d-none");
...
(async () => {
	...
	// const keyRing = await browser.runtime.sendMessage({
	// 	action: "load_keyring",
	// });
	...
	/*} catch (err) {
		...
		//alert("Error loading options page: " + err);
	}*/
})();
